...
Font::~Font() { glDeleteTextures(1, &textureAtlasID); }
...
void Font::loadFont(const std::string &name, int size) {
    ...
    if (!face) {
        std::cerr
            << "[Font] Failed to load the font face. Ensure the file path is correct."
            << std::endl;
        return;
    }
...
void Font::createTextureAtlas() {
    ...
    atlasWidth = 512;
    atlasHeight = 512;
    std::cout << "[Font] Creating texture atlas with size: " << atlasWidth << "x"
              << atlasHeight << "\n";
    ...
    // // Clear any residual errors from previous initialization steps
    ...
    // // force 1-byte alignment for FreeType glyph data
    // glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
    ...
    for (unsigned char c = 32; c < 127; ++c) {
        ...
        if (glyphIndex == 0) {
            std::cerr << "[Font] WARNING: Character not found in font: " << c << " ("
                      << static_cast<int>(c) << ")" << std::endl;
            continue;
        }
        if (FT_Load_Char(face, c, FT_LOAD_RENDER)) {
            std::cerr << "[Font] WARNING: Failed to load character: " << c << " ("
                      << static_cast<int>(c) << ")" << std::endl;
            continue;
        }
        FT_GlyphSlot g = face->glyph;
        if (g->bitmap.buffer == nullptr || g->bitmap.width == 0 || g->bitmap.rows == 0) {
            // std::cerr << "Warning: Glyph '" << c << "' has no valid bitmap data!" <<
            // std::endl; continue; // Skip this character (comment out to allow for
            // spaces)
        }
        if (glyphs.find(c) != glyphs.end()) {
            std::cerr << "[Font] ERROR: Character " << c
                      << " already exists in glyph map!" << std::endl;
            break;
        }
        ...
        if (offsetX + static_cast<int>(g->bitmap.width) > atlasWidth) {
            ...
            std::cout << "[Font] Reached atlas width limit: " << offsetX << " + "
                      << g->bitmap.width << " = "
                      << offsetX + static_cast<int>(g->bitmap.width) << " > "
                      << atlasWidth << ". Starting new row. Height: " << offsetY << "/"
                      << atlasHeight << "\n";
            offsetX = 0;      // reset
        ...
        if (offsetY + static_cast<int>(g->bitmap.rows) > atlasHeight) {
            std::cerr << "[Font] ERROR: Exceeded atlas height! " << offsetY << " + "
                      << g->bitmap.rows << " = "
                      << offsetY + static_cast<int>(g->bitmap.rows) << " > "
                      << atlasHeight << ". Texture atlas too small!" << std::endl;
            break;
        }
        ...
        for (int y = 0; y < (int)bitmap.rows; ++y) {
            std::memcpy(&flippedBitmap[y * bitmap.width],
                        &bitmap.buffer[(bitmap.rows - 1 - y) * bitmap.width],
                        bitmap.width);
        }

        // Copy glyph bitmap to the atlas
        glTexSubImage2D(GL_TEXTURE_2D, 0, offsetX, offsetY, g->bitmap.width,
                        g->bitmap.rows, GL_RED, GL_UNSIGNED_BYTE, flippedBitmap.data());
        ...
        // glyphs[c].offsetY = (ascenderPx - g->bitmap_top);// - (g->bitmap.rows -
        // descenderPx); maxAscent = int(face->ascender * (face->size->metrics.y_scale /
        // 65536.0)) >> 6; maxDescent = int(abs(face->descender *
        ...
        // std::cout << "Loaded character: " << c << " (" << static_cast<int>(c) << ")
        // Asc: " << ascenderPx << " Desc: " << descenderPx << std::endl; std::cout <<
        // "Successfully loaded glyph: " << c << std::endl;
    }

    glBindTexture(GL_TEXTURE_2D, 0);

    wastedArea = atlasWidth * atlasHeight - totalglyphArea;
    minWastedArea = wastedArea - (atlasHeight * (atlasWidth - maxWidth)) -
                    (maxWidth * (atlasHeight - offsetY));
...
const Glyph &Font::getGlyph(char c) const {
    ...
    std::cerr << "[Font] WARNING: Glyph not found for character '" << c << "' ("
              << static_cast<int>(c) << ")" << std::endl;
    return fallback;
}
