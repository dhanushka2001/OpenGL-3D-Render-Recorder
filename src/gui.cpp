...
namespace GUI {
    namespace { // anonymous namespace (encapsulation) evaluated once at program
        ...
        constexpr const char *presets[] = {"ultrafast", "superfast", "veryfast",
                                           "faster",    "fast",      "medium",
                                           "slow",      "slower"};
    ...
    void Render(Encoder *encoder) {
        ...
        int imgui_height =
            Settings::libx264 ? 542 : 559; // new line = +20 height. libx264
                                           // height = h264_mf height - 16.
#ifdef _WIN32
        ImGui::SetNextWindowPos(ImVec2(padding, padding), ImGuiCond_Once);
#endif /* _WIN32 */
#ifdef __linux__
        ImGui::SetNextWindowPos(ImVec2(padding, padding + imgui_height),
            ...
            ImGui::BeginDisabled();
        {
            ...
            if (Settings::libx264) {
                ...
                // h264_mf settings
                static int gopSize = Settings::
                    g_gop_size; // e.g. 10–120 // smaller GOP=>more I-frames:
                                // better seeking/error recovery, faster
                                // previewing, but larger file size. for
                                // real-time/streaming: GOP = 2 x framerate.
                static int maxBFrames = Settings::g_max_b_frames; // 0–5

                ImGui::PushItemWidth(100.0f); // Set desired width in pixels
                int mbps = static_cast<int>(Settings::g_bit_rate /
                                            1'000'000); // Convert to Mbps
                if (ImGui::SliderInt("Bitrate (Mbps)", &mbps, 10, 100, "%d",
                                     ImGuiSliderFlags_AlwaysClamp)) {
                    Settings::g_bit_rate = static_cast<int64_t>(mbps) *
                                           1'000'000; // Convert back to bps
                }
            ...
            if (ImGui::Combo("Framerate", &selectedFramerateIdx, framerates,
                             IM_ARRAYSIZE(framerates))) {
                Settings::framerate = (selectedFramerateIdx == 0) ? 30 : 60;
            }

            // bool isLaptopMode = Settings::laptop_mode;
            ImGui::Checkbox("Laptop mode (deep copy frames, thread-safe)",
...
#ifdef _WIN32
        ImGui::SetNextWindowPos(ImVec2(SCR_WIDTH - implot_width, padding),
                                ImGuiCond_Once);
#endif /* _WIN32 */
#ifdef __linux__
        ImGui::SetNextWindowPos(
            ImVec2(SCR_WIDTH - implot_width, padding + implot_height),
            ImGuiCond_FirstUseEver);
#endif /* __linux__ */
        ImGui::SetNextWindowSize(ImVec2(implot_width, implot_height),
                                 ImGuiCond_Once);
